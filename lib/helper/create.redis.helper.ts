...
export function createProducerSingleRedis(
  moduleOpts: IORedisModuleOptions,
  key: string | symbol,
) {
  const { readyLog, errorLog, onClientCreated, producer, consumer } =
    moduleOpts;
  const { url, path, ...commonOpitons } = (producer ||
    consumer) as SingleRedisOptions;
...
export function createConsumerSingleRedis(
  moduleOpts: IORedisModuleOptions,
  key: string | symbol,
) {
  const { readyLog, errorLog, onClientCreated, producer, consumer } =
    moduleOpts;
  const { url, path, ...commonOpitons } = (consumer ||
    producer) as SingleRedisOptions;
