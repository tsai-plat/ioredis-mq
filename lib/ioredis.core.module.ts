...
import { IORedisModuleAsyncOptions, IORedisModuleOptions, IORedisModuleOptionsFactory } from "./interfaces";
import { generateModuleId, validateOptionsType, validMQChannelNames } from "./utils";
import { createOptionsProvider, createRedisClient, createRedisMQClient, mergedOptionsProvider } from "./helper/core.provider.helper";
...
export class IORedisCoreModule {
    ...
    private static createAsyncOptionsProvider(options:IORedisModuleAsyncOptions):Provider{
        ...
        const inject = [
            (options.useClass||options.useExisting) as Type<IORedisModuleOptionsFactory>
        ]
