...
import { DEFAUL_PUB_CHANNEL, IOREDIS_MERGED_OPTIONS_TOKEN, IOREDIS_MQ_TOKEN, MQ_HANDLER_PROPS_SUBCHANNEL, MQ_MESSAGE_CHANNEL } from "../ioredis.constants";
import { IoRedisModuleError } from "../errors";
import { IoRedisModuleOptions, MQChannelType, MQHandlerFn, MQMessageType } from "../interfaces";
import { buildRedisKey, createMessageId, genMQClientId, LOCK_KEY_ROOT_SCOPE, validMQChannelNames } from "../utils";
...
export class RedisMQService implements OnModuleInit {
    ...
    private readonly channelMap = new Map<MQChannelType|string,Array<MQHandlerFn>>()
    private  maxListeners = 3
    private debug = true
    private ttl: number = DEFAULT_LOCK_TTL;
    private skipSelf:boolean = true
    ...
    constructor(
        ...
        const {verbose =false,ttl=DEFAULT_LOCK_TTL,maxListeners=defaultMaxListeners,skipSelf=true} = mqOptions||{}
        this.debug = verbose
        this.ttl = ttl
     ...
     * @param channel 
     */
    async publishMessage<T=any>(data:T,channel:MQChannelType=DEFAUL_PUB_CHANNEL){
        ...
        this.logged('log',`Publish msg ${channel}\n${JSON.stringify(messageObj,null,2)}`)
    ...
    private async handleMessage(channel:string,message:string){
        ...
        if(this.skipSelf && sender === this.mqid){
            this.logged('warn',`Unsolicited messages id[${id}] are ignored.`)
            return;
        }

        const handlers:Array<MQHandlerFn<MQMessageType>> = this.channelMap.get(channel)??[]
        ...
        if(await this.acquireLock(lockedKey)){
            try{
                if(!handlers.length){
                    this.logged('warn',`Recived Message ID ${id},but no registered handler\n\n ${JSON.stringify({...others,channel,},null,2)}`)
                    return 
                }
    ...
    private async processByRegistered(channel:string,json:MQMessageType,handlers:Array<MQHandlerFn<MQMessageType>>){
        for(let i=0;i<handlers.length;i++){
            try {
                const rk = Reflect.get<MQHandlerFn,string>(handlers[i],MQ_HANDLER_PROPS_SUBCHANNEL)
                this.logged('warn',`Handler >>>> reflect>>>> ${rk}`)
    ...
    registHandler(channel:MQChannelType,handler:MQHandlerFn){
        ...
        if(handlers.length>=this.maxListeners){
            throw new IoRedisModuleError(`MQ handlers required max ${this.maxListeners},can not registered.`)
        }
        if(handlers.findIndex((v)=>v===handler) ===-1){
            this.logged('log',`Will registered ${handler.name} for channel [${channel}]`)
    ...
    private logged(level:'log'|'warn'|'error'='log',message:string,ex?:any,){
        if(!this.debug)
            return ;
     ...
     * @returns boolean
     */
    async removeHandlers(channel:MQChannelType,handler?:MQHandlerFn):Promise<boolean>{
        if(!this.channelMap.has(channel))
            return true;
    ...
    private paserMQJsonMessage(
        message: string,
      ): MQMessageType<any> | null {
        ...
        try {
          ...
          this.logger.warn(`Received message can not parsed JSON MQ message.`, e?.message);
          return null;
        }
    }
